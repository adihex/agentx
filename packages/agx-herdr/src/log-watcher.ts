...
import { AdpClient, nowHHMMSS, type LogLevel } from "@agentx/agx-core";
...
async function startLogWatcher() {
  ...
  client.onEvent((ev) => {
    ...
    if (ev.method === "Log.Entry") {
      ...
      printLog(nowHHMMSS(), (params.level as LogLevel) ?? "INFO", params.message);
    }
    ...
    if (ev.method === "Agent.StatusUpdate") {
      ...
      printLog(nowHHMMSS(), "INFO", `[${p.agentId}] → ${p.status} (${p.progress}%)`);
    }

    // Debugger responses
    if (ev.method === "Debugger.Response") {
      const p = ev.params as { result?: unknown };
      printLog(nowHHMMSS(), "CMD", `Response: ${JSON.stringify(p.result || p)}`);
    }
    ...
    if (ev.method === "Tool.Completed") {
      ...
      printLog(nowHHMMSS(), "INFO", `Tool completed: ${p.name} (${p.durationMs}ms)`);
    }
