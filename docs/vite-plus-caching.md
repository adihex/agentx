...
vp run @agentx/zettel#build
    ...
    └── @agentx/agx-core#build  (tsc)
            │
            ▼
        @agentx/zettel#build    (tsc -b && tsc -p tsconfig.server.json && vite build)
```
...
The one uncached task is `@agentx/pi-extension#build` which runs `echo` — correctly skipped by `vp` since zero-cost commands don't benefit from caching.
...
mise exec -- pnpm exec vp run @agentx/zettel#build

# Force a fresh build (ignore cache)
mise exec -- pnpm exec vp run @agentx/zettel#build --force
